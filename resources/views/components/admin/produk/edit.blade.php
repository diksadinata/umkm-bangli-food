...
<form method="post" action="{{ route('admin.update_produk') }}" enctype="multipart/form-data">
    ...
    <div class="modal-body">
        ...
        <div class="form-group mt-3">
                                ...
                                <select class="form-select" name="nama_restoran" required>
                                @foreach($kecamatan as $k)
                                <option value="{{ $k->id }}" {{ $edit->id == $k->id ? 'selected' : ''}}>{{ $k->kecamatan }}</option>
                                @endforeach
</select >
                                @error("nama_restoran")
                                    <span class="text-danger">{{ $message }}</span>
        ...
        <div class="form-group mt-3">
            ...
            <textarea class="form-control @error("lokasi") is-invalid @enderror" rows="8" name="lokasi" id="lokasi" placeholder="">{{$edit->lokasi}}</textarea>
        </div>
        <div class="form-group mt-3">
